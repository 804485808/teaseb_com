	...
	public function serviceDefaultOptionCanonical($arrayAttrids,$catid){
		...
		foreach($defaultOption as $name => $ov){
			...
			foreach($ov as $k => $value){
				...
				$newvids = $attrs + array($value['oid']=>$value['id']);
				
				$attrtag = $this->category_option->getCategoryOptionCanonical($newvids,$catid);
				$defaultOption[$name][$k]['title'] = $attrtag['title'];
				$defaultOption[$name][$k]['linkurl'] = $attrtag['linkurl'];
				$defaultOption[$name][$k]['titlelinkurl'] = $attrtag['titlelinkurl'];
			}
		}
