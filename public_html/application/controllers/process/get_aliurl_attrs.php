...
class Get_aliurl extends CI_Controller{
	function index(){
			...
			$tag = $this->uri->rsegment(3,'');
			$catid = $this->uri->rsegment(4,0);
			dump($tag);
			...
			$findattr = $this->comm->find("attrtag",array("tag"=>$tag,"catid"=>$catid));
			if(!$findattr){
				dump("not find attr");
				die();
			}
			$category = $this->comm->find("category",array("catid"=>$findattr['catid']));
		
			
			$searchword = preg_replace("/\s{2,}/"," ",$findattr['tag']." ".$category['catname']);
