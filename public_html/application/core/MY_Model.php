...
class MY_Model extends CI_Model{
    ...
    * 公共查询
    */
    public function SelectCommon($files,$link,$group='',$where,$order=array(),$limit='',$type=0){
        $this->db->select($files,false);

        //link
        if($link){

            $tempFrom = $this->_table." as t1";
            $this->db->from($tempFrom);
            $i = 2;
            foreach($link as $k=>$v){
                $tempArr = $this->_link1;
                if($tempArr[$k]){
                    ...
                    $temp = $tempArr[$k];
                    $two = "t1.".$temp['selfKey']."=t".$i.".".$temp['linkKey'];
        ...
        if($where) {
            foreach($where as $k=>$v){
                ...
                //in
                if(count($tempArr)>1 && in_array('in',$tempArr)){
                    $one = $tempArr[0];
                    if(!is_array($v)){
                        $two = explode(',',$v);
                    }else{
